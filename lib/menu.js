            ...
            connection.query('SELECT department_name FROM departments', function (err, results, fields) {
                ...
                inquirer
                    ...
                    .then((data) => {
                        ...
                        connection.connect((err) => {
                            if (err) throw err;
                            let sql = `INSERT INTO roles (title, salary) VALUES ('${data.addRole}', '${data.salary}') SELECT roles.title, roles.salary, departments.department_name FROM roles LEFT JOIN departments ON roles.department_id = departments.id ORDER BY roles.title`
